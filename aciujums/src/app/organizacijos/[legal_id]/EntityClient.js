...
import { getEntityDataWithCache } from '@/lib/indexedDB';

export default function EntityClient({ initialData = null }) {
    const { legal_id } = useParams();
    const [data, setData] = useState(initialData);
    const [loading, setLoading] = useState(!initialData);

    useEffect(() => {
        if (!legal_id || !/^\d{1,15}$/.test(legal_id)) return;
        // If we already have SSR data, save it to IndexedDB for future navigations
        if (initialData) {
            getEntityDataWithCache(legal_id).catch(() => {});
            return;
        }
        setLoading(true);
        getEntityDataWithCache(legal_id)
            .then((result) => {
                setData(result);
                setLoading(false);
            })
            .catch(() => setLoading(false));
    }, [legal_id]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    const finances = [...(data.finances || [])].sort((a, b) => a.year - b.year);
    const latestFinance = finances.findLast((f) => f.record_dt);

    return (
        <div className="container pt-12 mt-10">
            <h2>{info.ja_pavadinimas}</h2>

            {info.nvo_statusas === 1 && (
                <div className="tooltip-wrapper">
                    <button type="button" className="badge badge-pill badge-primary tooltip-anchor" aria-describedby="tooltip-nvo">NVO</button>
            ...
            {info.par_gav_statusas === 0 && (
                <div className="tooltip-wrapper">
                    <button type="button" className="badge badge-pill badge-danger tooltip-anchor" aria-describedby="tooltip-npar">Nėra paramos gavėjas</button>
                    <span id="tooltip-npar" role="tooltip" className="tooltip">Juridiniam asmeniui nėra suteiktas paramos gavėjo statusas</span>
                </div>
            )}

            <h3 className="mt-12 mb-2">Organizacijos bendra informacija</h3>
            <div className="flex flex-wrap">
                <div className="w-full md:w-1/2">
                    <ul className="info-list">
                        <li className="mb-6">
                            <span className="block font-bold info-name info-name--entity">Įmonės kodas</span>
                            <span className="block font-bold info-value info-value--entity">{legal_id}</span>
                        </li>
                        <li className="mb-6">
                            <span className="block font-bold info-name info-name--entity">Registracijos data</span>
                            <span className="block font-bold info-value info-value--entity">{info.ja_reg_data}</span>
                        </li>
                        <li className="mb-6">
                            <span className="block font-bold info-name info-name--entity">Teisinė forma</span>
                            <span className="block font-bold info-value info-value--entity">{info.form_pavadinimas}</span>
                        </li>
                        <li className="mb-6">
                            <span className="block font-bold info-name info-name--entity">Teisinis statusas</span>
                            <span className="block font-bold info-value info-value--entity">{info.stat_pavadinimas}</span>
                        </li>
                    </ul>
                </div>
                <div className="w-full md:w-1/2">
                    <ul className="info-list">
                        {info.nvo_statusas === 1 && (
                            <li className="mb-6">
                                <span className="block font-bold info-name info-name--entity">NVO statuso įsigaliojimo data</span>
                                <span className="block font-bold info-value info-value--entity">{info.paramos_gav_nuo}</span>
                            </li>
                        )}
                        {info.par_gav_statusas === 1 && (
                            <li className="mb-6">
                                <span className="block font-bold info-name info-name--entity">Paramos gavėjo statuso įsigaliojimo data</span>
                                <span className="block font-bold info-value info-value--entity">{info.paramos_gav_nuo}</span>
                            </li>
                        )}
                    </ul>
                ...
                <table className="table">
                    ...
                    <tbody>
                        {finances.map((entry) => (
                            <tr key={entry.year}>
                                <td>{entry.year}</td>
                                <td>{entry.total_funders}</td>
                                <td>
                                    {entry.total_funds.toLocaleString('lt-LT', {
            ...
            <div className="mt-2 mb-2">
                <span className="block text-gray-500">
                    Šaltinis:{' '}
                    <Link href="https://www.vmi.lt/evmi/paramos-statistika?lang=lt" target="_blank" rel="noopener noreferrer">
                        VMI
                    </Link>
                </span>
                {latestFinance?.record_dt && (
                    <span className="block text-gray-500">Duomenys atnaujinti: {latestFinance.record_dt}</span>
                )}
            </div>
        </div>
    );
