...
// functions/downloaders/rc-jar-no-fa/index.mjs
import { S3Client, PutObjectCommand } from "@aws-sdk/client-s3";
import https from "https";
var client = new S3Client({ region: "eu-central-1" });
var fileName = "JAR_NEPATEIKE_FA_UZ_PRAEJUSIUS.csv";
var handler = /* @__PURE__ */ __name(async (event, context) => {
  try {
    const dateParts = (/* @__PURE__ */ new Date()).toISOString().split("T")[0].split("-");
    const year = dateParts[0];
    const month = dateParts[1];
    const objectKey = `registru_centras/jar_nepateike_finansiniu_ataskaitu/year=${year}/month=${month}/${fileName}`;
    const options = {
