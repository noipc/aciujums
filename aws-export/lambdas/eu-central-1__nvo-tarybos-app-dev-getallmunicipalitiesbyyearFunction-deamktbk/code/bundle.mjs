...
var handler = /* @__PURE__ */ __name(async (event) => {
  if (process.env.SST_DEV) {
    ...
    const filePath = path.resolve(root, "data", "all_municipalities_2025.json");
  ...
  const year2 = parseInt(event.queryStringParameters?.year);
  if (!year2) {
    return { statusCode: 400, body: JSON.stringify({ error: "Year is required" }) };
  }
  try {
    const command = new QueryCommand2({
      ...
      KeyConditionExpression: "#yr = :yearValue",
      ExpressionAttributeNames: { "#yr": "year" },
      ExpressionAttributeValues: { ":yearValue": year2 }
    });
    const response = await docClient.send(command);
    const result = response.Items.map((item) => ({
      municipality: item.municipality_name,
      slug: item.municipality,
      region: item.region,
      year: item.year,
      nvo_status: item.nvo_status,
      link: item.link,
      council_chairperson: item.council_chairperson,
      chairperson_organisation: item.organisation,
      contact_full_name: item.contact_full_name,
