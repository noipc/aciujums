...
async function handler() {
  ...
  try {
    ...
    return {
      statusCode: 200,
      // Return 0 if never updated, ensuring first run always fetches
      body: JSON.stringify({ lastUpdated: response.Item?.lastUpdated || 0 })
    };
  } catch (e6) {
    return { statusCode: 500, body: JSON.stringify({ error: e6.message }) };
  }
}
