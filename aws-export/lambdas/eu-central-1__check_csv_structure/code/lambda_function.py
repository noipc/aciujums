...
def log_validation_result(batch_id, file_key, status, error_message = "", detected_delimeter):
    ...
    if detected_delimiter:
        item["detected_delimeter"] = detected_delimiter

    table.put_item(Item=item)

def extract_batch_id_from_key(key):
    # Example: "registru_centras/jar/year=2025/month=08/..."
    try:
        parts = key.split('/')
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if required_columns is None:
            ...
            log_validation_result(batch_id, key, "INVALID", error_msg, null)
        ...
        # Check if all files are valid, trigger processing
        if check_all_valid(batch_id):
            trigger_processing_event(batch_id)
